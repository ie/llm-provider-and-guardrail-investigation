...
export default function App() {
    ...
    async function loadVercelModelsOptions() {
      try {
        ...
        const languageOnlyAndUnder1Dollar = availableModels.models.filter(
          (m) =>
            ['amazon', 'google', 'openai', 'nvidia'].includes(
              m.id.split('/')[0],
            ) &&
  ...
  return (
    <GlobalStylesScope themeDefinition={theme}>
      <IconButton
        title={isLight ? "Toggle dark" : "Toggle light"}
        icon={isLight ? <SVGHomeFilled /> : <SVGHome />}
        variant={isLight ? 'default' : 'subtle'}
        onClick={() => setIsLight(!isLight)}
        className="fixed top-4 right-4 z-50"
      />
      <Chat vercelModels={modelOptions} />
    </GlobalStylesScope>
  )
}
