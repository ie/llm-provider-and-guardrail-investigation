...
import { bedrockGuardrail } from './bedrockGuardrail.js'
import { ossSafeguard } from './ossSafeguard.js'
...
export const SAFETY_CHECKS = {
    ...
    'bedrock-guardrail': bedrockGuardrail,
    'oss-safeguard': ossSafeguard,
}
...
// Accepts one name or a list. A list layers checks: they run in order and stop at the
// first block, so a later check never runs once the verdict is already decided.
export async function applySafety(names, text, options) {
    let result = { blocked: false }

    for (const name of Array.isArray(names) ? names : [names]) {
        const check = SAFETY_CHECKS[name]
        if (!check) throw new Error(`Unknown safety check: ${name}`)
        ...
        result = await check(text, options)
