...
import { applySafety, refusal } from '../safety/index.js'
import { summariseTrace } from '../safety/assessment.js'
...
// the `safety` arg still drives the registry checks on top.
export async function chat({ message, history, modelId, safety, inlineGuardrail = false }) {
  ...
  const inputCheck = await applySafety(safety, message, { documents: chunks, source: 'INPUT' })
  ...
  if (converse.stopReason === 'guardrail_intervened') {
    console.log(
      `[safety] name=bedrock-inline source=CONVERSE blocked=true grounded=${inlineGrounded} trace=${JSON.stringify(summariseTrace(converse.trace?.guardrail))}`,
    )
  ...
  const outputCheck = await applySafety(safety, reply, {
